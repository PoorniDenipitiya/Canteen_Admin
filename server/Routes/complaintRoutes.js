...
router.put("/:id/status", async (req, res) => {
  try {
    ...
    console.log(
      `[Complaint] Updating status for complaintId: ${req.params.id} to: ${status}`
    );
    ...
    if (status === "On MO Investigation") {
      ...
      if (moUser && moUser.email) {
        ...
        transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
            console.error(
              `[Complaint] Error sending email to Medical Officer:`,
              error
            );
          } else {
            console.log(
              `[Complaint] Email sent to Medical Officer:`,
              info.response
            );
          }
        });
        ...
        console.warn(
          `[Complaint] No Medical Officer user found or missing email.`
        );
      }
    }
    ...
    if (status === "MO Investigation Completed") {
      if (complaint.action && complaint.action !== "-----") {
        ...
        console.log(
          `[Complaint] Email not sent for status '${status}' because action is not selected.`
        );
      }
    }
    ...
    if (user && user.email) {
      if (statusRequiresAction && complaint.action === "Refund") {
        const canteenOwner = await AdminUser.findOne({
          canteenName: complaint.canteenName,
        });
        if (canteenOwner && canteenOwner.email) {
          const mailOptions = {
            ...
            text: `Dear Canteen Owner,\n\nA complaint for your canteen requires a refund.\n\n${complaintDetails}\n\n**Note: Within 3 days you have to refund the order payment again to the Admin.**`,
          };
          transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
              console.error(
                `[Complaint] Error sending refund email to canteen owner:`,
                error
              );
            } else {
              console.log(
                `[Complaint] Refund email sent to canteen owner:`,
                info.response
              );
            }
          });
          ...
          console.warn(
            `[Complaint] No canteen owner found or missing email for canteen: ${complaint.canteenName}`
          );
        }
      }
      if (
        !statusRequiresAction ||
        (complaint.action && complaint.action !== "-----")
      ) {
        let note = "";
        if (statusRequiresAction && complaint.action === "Refund") {
          note =
        ...
        transporter.sendMail(mailOptions, (error, info) => {
          if (error) {
            console.error(
              `[Complaint] Error sending email to ${user.email}:`,
              error
            );
          } else {
            console.log(
              `[Complaint] Email sent successfully to: ${user.email}`,
              info.response
            );
          }
        });
        ...
        console.log(
          `[Complaint] Email not sent to user for status '${status}' because action is not selected.`
        );
      }
    } else {
      console.warn(
        `[Complaint] User not found or missing email for userId: ${complaint.userId}`
      );
    }
...
router.put("/:id/action", async (req, res) => {
  try {
    ...
    console.log(
      `[Complaint] Updating action for complaintId: ${req.params.id} to: ${action}`
    );
    const complaint = await Complaint.findByIdAndUpdate(
      req.params.id,
      { action },
